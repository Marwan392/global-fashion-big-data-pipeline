...
st.set_page_config(
    page_title="Global Fashion Retail Executive Dashboard",
    page_icon="🛍️",
    layout="wide"
)
...
if page == "Overview & Insights":
    ...
    st.markdown(
        """
        Analytics dashboard powered by a distributed Big Data pipeline:
    ...
    if data:
        ...
        col1.metric(
            "Total Transactions",
            f"{data.get('total_transactions', 0):,}"
        )
        ...
        col5.metric(
            "Profit Margin",
            f"{data.get('profit_margin', 0) * 100:.2f}%"
        )

        col6.metric(
            "Unique Orders",
            f"{data.get('unique_orders', 0):,}"
        )
...
# ============================================================
    ...
    if data:
        ...
        if churn_data:
            ...
            st.subheader(
                "Historical Customer Churn Distribution"
            )
...
# ============================================================
    ...
    if summary:
        ...
        col2.metric(
            "Predicted to Churn",
            f"{summary.get('predicted_churn_customers', 0):,}"
        )


        col3.metric(
            "Average Churn Probability",
            f"{summary.get('average_churn_probability', 0) * 100:.2f}%"
        )
    ...
    if high_risk_data:
        ...
        # Show useful columns only if they exist
